...
for jj=1:length(stiffness)
    ...
    plot(t,subpos);
    xlabel('time (s)');
    TITLE = sprintf('Stiffness Set Number %d',jj);
    ...
    elseif jj == length(stiffness)
        ...
        TITLE = sprintf('Stiffness Set Number %d and %d vs time',1,jj);
...
ax = gca;
exportgraphics(ax,'Stiff vs Retro 1.png')
...
ax = gca;
exportgraphics(ax,'Stiff vs Tract 1.png')
...
axes('NextPlot', 'add')
yyaxis left
semilogx(stiffness,-avtrac)%,stiffness, retro)
ylabel('Mean traction force (pN)')
yyaxis right
semilogx(stiffness, retro)
ylabel('Mean retrograde flow (nm/s)')
...
ax = gca;
exportgraphics(ax,'Stiff vs Tract and Retro 2.png')
